...
**③ A/B/C/D 四个角色的完整分析卡片**（可展开），每张卡片给出该维度的推理链、结论和一句话笔记。例如技术面 C 的实际输出片段：
...
> 完整 HTML 报告是暗色主题、可交互展开的，上面只是文字摘录。你自己跑一次 `python analyze.py 002475` 就能得到同样结构的报告。
...
> ⚠️ `.env` 里是你的私人密钥，项目已配置好不会上传到 GitHub，放心填。
